...
namespace Property365.Web.Components
{
    ...
    public class DropDownBase<T> : DataBoundFormComponent<T>
    {
        ...
        private async ValueTask<Microsoft.AspNetCore.Components.Web.Virtualization.ItemsProviderResult<object>> LoadItems(Microsoft.AspNetCore.Components.Web.Virtualization.ItemsProviderRequest request)
        {
            ...
            {
                await LoadData.InvokeAsync(new Property365.Web.Components.LoadDataArgs() { Skip = request.StartIndex, Top = request.Count, Filter = await JSRuntime.InvokeAsync<string>("Property365Components.getInputValue", search) });
            }
        ...
        protected async System.Threading.Tasks.Task ClearAll()
        {
            ...
            searchText = null;
            await JSRuntime.InvokeAsync<string>("Property365Components.setInputValue", search, "");
        ...
        protected string OpenPopupScript()
        {
            ...
            return $"Property365Components.togglePopup(this.parentNode, '{PopupID}', true);Property365Components.focusElement('{SearchID}');";
        }
        ...
        protected string OpenPopupScriptFromParent()
        {
            ...
            return $"Property365Components.togglePopup(this, '{PopupID}', true);Property365Components.focusElement('{SearchID}');";
        }
        ...
        protected virtual async System.Threading.Tasks.Task OpenPopup(string key = "ArrowDown", bool isFilter = false, bool isFromClick = false)
        {
            ...
            await JSRuntime.InvokeVoidAsync("Property365Components.togglePopup", Element, PopupID, true);
        ...
        private async System.Threading.Tasks.Task HandleKeyPress(Microsoft.AspNetCore.Components.Web.KeyboardEventArgs args, bool isFilter = false)
        {
            ...
            if (!args.AltKey && (key == "ArrowDown" || key == "ArrowLeft" || key == "ArrowUp" || key == "ArrowRight"))
            {
                try
                {
                    ...
                    var newSelectedIndex = await JSRuntime.InvokeAsync<int>("Property365Components.focusListItem", search, list, key == "ArrowDown" || key == "ArrowRight", currentViewIndex);
                    ...
                    {
                        selectedIndex = await JSRuntime.InvokeAsync<int>("Property365Components.focusListItem", search, list, key == "ArrowDown", currentViewIndex);
                    }
                }
                catch (Exception)
                {
            ...
            else if (Multiple && key == "Space")
            {
                ...
                {
                    await JSRuntime.InvokeAsync<string>("Property365Components.setInputValue", search, $"{searchText}".Trim());
                    await OnSelectItem(items.ElementAt(selectedIndex), true);
                }
            }
        ...
        async Task DebounceFilter()
        {
            if (!LoadData.HasDelegate)
            {
                searchText = await JSRuntime.InvokeAsync<string>("Property365Components.getInputValue", search);
                _view = null;
                if (IsVirtualizationAllowed())
                {
#if NET5_0_OR_GREATER
            ...
            await JSRuntime.InvokeAsync<string>("Property365Components.repositionPopup", Element, PopupID);
        }
        ...
        internal virtual async System.Threading.Tasks.Task<LoadDataArgs> GetLoadDataArgs()
        {
            ...
            if (AllowVirtualization)
            {
                return new Property365.Web.Components.LoadDataArgs() { Skip = 0, Top = PageSize, Filter = await JSRuntime.InvokeAsync<string>("Property365Components.getInputValue", search) };
            }
            else
            {
                return new Property365.Web.Components.LoadDataArgs() { Filter = await JSRuntime.InvokeAsync<string>("Property365Components.getInputValue", search) };
            }
#else
            return new Property365.Web.Components.LoadDataArgs() { Filter = await JSRuntime.InvokeAsync<string>("Property365Components.getInputValue", search) };
#endif
        ...
        public override async Task SetParametersAsync(ParameterView parameters)
        {
            ...
            {
                await JSRuntime.InvokeVoidAsync("Property365Components.destroyPopup", PopupID);
            }
        }
