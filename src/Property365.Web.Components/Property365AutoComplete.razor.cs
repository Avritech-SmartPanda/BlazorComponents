...
{
    /// <summary>
    /// Property365AutoComplete component.
    /// </summary>
    /// <example>
    /// <code>
    /// &lt;Property365AutoComplete Data=@customers TextProperty="CompanyName" Change=@(args => Console.WriteLine($"Selected text: {args}")) /&gt;
    /// </code>
    /// </example>
    public partial class Property365AutoComplete : DataBoundFormComponent<string>
    {
        /// <summary>
        ...
        protected async Task OnFilterKeyPress(KeyboardEventArgs args)
        {
            ...
            if (key == "ArrowDown" || key == "ArrowUp")
            {
                try
                {
                    selectedIndex = await JSRuntime.InvokeAsync<int>("Property365Components.focusListItem", search, list, key == "ArrowDown", selectedIndex);
                }
                catch (Exception)
                {
                    //
        ...
        {
            var value = await JSRuntime.InvokeAsync<string>("Property365Components.getInputValue", search);
            ...
            else
            {
                ...
                await InvokeAsync(() => { LoadData.InvokeAsync(new LoadDataArgs() { Filter = customSearchText }); });
            }
        }
        ...
        private string OpenScript()
        {
            ...
            return $"Property365Components.openPopup(this.parentNode, '{PopupID}', true)";
        }
