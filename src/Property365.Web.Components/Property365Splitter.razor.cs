...
{
    /// <summary>
    /// Property365Splitter component.
    /// </summary>
    public partial class Property365Splitter : Property365Component
    {
        ...
        internal List<Property365SplitterPane> Panes = new List<Property365SplitterPane>();

        /// <summary>
        /// Adds the pane.
        /// </summary>
        /// <param name="pane">The pane.</param>
        public void AddPane(Property365SplitterPane pane)
        {
        ...
        /// </summary>
        /// <param name="pane">The pane.</param>
        public void RemovePane(Property365SplitterPane pane)
        {
        ...
        internal Task ResizeExec(MouseEventArgs args, int paneIndex)
        {
            ...
            return JSRuntime.InvokeVoidAsync("Property365Components.startSplitterResize",
                UniqueID,
                Reference,
                pane.UniqueID,
        ...
        [JSInvokable("Property365Splitter.OnPaneResized")]
        public async Task OnPaneResized(int paneIndex, double sizeNew, int? paneNextIndex, double? sizeNextNew)
        {
            ...
            {
                var arg = new Property365SplitterResizeEventArgs { PaneIndex = pane.Index, Pane = pane, NewSize = sizeNew };
            ...
            if (paneNextIndex.HasValue)
            {
                ...
                {
                    var arg = new Property365SplitterResizeEventArgs { PaneIndex = paneNext.Index, Pane = paneNext, NewSize = sizeNextNew.Value };
                    await Resize.InvokeAsync(arg);
        ...
        internal async Task CollapseExec(object args, int paneIndex, string paneId)
        {
            ...
            if (paneNext != null && paneNext.Collapsible && paneNext.IsLast && paneNext.Collapsed)
            {
                if (Expand.HasDelegate)
                {
                    var arg = new Property365SplitterEventArgs { PaneIndex = paneNext.Index, Pane = paneNext };
            ...
            else
            {
                ...
                {
                    var arg = new Property365SplitterEventArgs { PaneIndex = pane.Index, Pane = pane };
        ...
        [Parameter]
        public EventCallback<Property365SplitterEventArgs> Collapse { get; set; }
        ...
        [Parameter]
        public EventCallback<Property365SplitterEventArgs> Expand { get; set; }
        ...
        [Parameter]
        public EventCallback<Property365SplitterResizeEventArgs> Resize { get; set; }
    }
}
